...
func (cr *UserHandler) VeiwAddress(ctx *gin.Context) {
	...
	Address, err := cr.userUseCase.VeiwAdress(ctx, userID)
	if err != nil {
		ctx.JSON(http.StatusBadRequest, response.Response{
			StatusCode: 400,
...
func (cr *UserHandler) AddToWishList(ctx *gin.Context) {
	...
	err = cr.userUseCase.AddToWishList(ctx, wishList)
	if err != nil {
		ctx.JSON(http.StatusBadRequest, response.Response{
			StatusCode: 400,
			Message:    "something Went Wrong",
...
// @Failure 400 {object} response.Response{} "invalid input"
func (cr *UserHandler) RemoveFromWishList(ctx *gin.Context) {
	paramsId := ctx.Param("id")
	productID, err := strconv.Atoi(paramsId)
	if err != nil {
		ctx.JSON(http.StatusBadRequest, response.Response{
	...
	if err := cr.userUseCase.RemoveFromWishList(ctx, wishList); err != nil {
		ctx.JSON(http.StatusBadRequest, response.Response{
			StatusCode: 400,
			Message:    "something Went Wrong",
			Data:       nil,
...
func (cr *UserHandler) GetWishList(ctx *gin.Context) {
	userID, err := utilhandler.GetUserIdFromContext(ctx)
	if err != nil {
		ctx.JSON(http.StatusBadRequest, response.Response{
			StatusCode: 400,
	...
	wishList, err := cr.userUseCase.ListWishlist(ctx, uint(userID))
	if err != nil {
		ctx.JSON(http.StatusBadRequest, response.Response{
			StatusCode: 400,
			Message:    "something Went Wrong",
