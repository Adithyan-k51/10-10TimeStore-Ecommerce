...
func (cr *ProductHandler) Addcategory(c *gin.Context) {
	...
	NewCategory, err := cr.ProductUsecase.Addcategory(c.Request.Context(), newcategory)
	if err != nil {
		c.JSON(http.StatusBadRequest, response.Response{
			StatusCode: 400,
...
func (cr *ProductHandler) UpdateCategory(c *gin.Context) {
	...
	updatedCategory, err := cr.ProductUsecase.UpdateCategory(c.Request.Context(), category, id)
	if err != nil {
		c.JSON(http.StatusBadRequest, response.Response{
			StatusCode: 400,
			Message:    "Can't update category",
...
func (cr *ProductHandler) DeleteCategory(c *gin.Context) {
	...
	err = cr.ProductUsecase.DeleteCategory(c.Request.Context(), id)
	if err != nil {
		c.JSON(http.StatusBadRequest, response.Response{
			StatusCode: 400,
			Message:    "Can't dlt category",
			Data:       nil,
			Errors:     err.Error(),
		})
		return
...
func (cr *ProductHandler) ListCategories(c *gin.Context) {
	categories, err := cr.ProductUsecase.Listallcategory(c.Request.Context())
	if err != nil {
		c.JSON(http.StatusBadRequest, response.Response{
			StatusCode: 400,
...
func (cr *ProductHandler) SearchProducts(c *gin.Context) {
	query := c.Query("q") // Example: /products/search?q=k
	...
	products, err := cr.ProductUsecase.SearchProducts(c.Request.Context(), query)
	if err != nil {
...
func (cr *ProductHandler) FilterProductsByPrice(c *gin.Context) {
	var min, max *float64

	if minStr := c.DefaultQuery("min", "0"); minStr != "" {
	...
	if maxStr := c.DefaultQuery("max", "999999"); maxStr != "" {
	...
	products, err := cr.ProductUsecase.GetProductsByPriceRange(min, max)
	if err != nil {
