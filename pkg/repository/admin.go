...
func (c *AdminDB) FindAllUser(ctx context.Context, pagination requests.Pagination) ([]response.UserValue, error) {
	// Validate pagination parameters
	if pagination.Page < 1 || pagination.PerPage < 1 {
		return nil, fmt.Errorf("invalid pagination parameters: page and per_page must be positive")
	}
	...
	err := c.DB.WithContext(ctx).Raw(query, limit, offset).Scan(&users).Error
	if err != nil {
